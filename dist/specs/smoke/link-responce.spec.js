...
function getLinks(url) {
    return browser.url(url).pause(1000 // TODO: have to comeup with JS/DOM watch
    ).getAttribute('<a />', 'href');
}
...
getAllLinks(customLinksProvider.data).then(allLinks => {
        ...
        describe(`Check all hrefs in ${_url} `, () => {
            ...
            _allLinks.forEach((_url, i) => {
                it(`#${i}   ${_url} link should return 2xx or 3xx status code`, () => {
                    return httpGetFullResponse(_url).then(res => {
